package microfont

import (
	"image"
	"iter"
	"maps"

	"golang.org/x/image/font"
	"golang.org/x/image/math/fixed"
)

// Face56vw is a variable width bitmap font with 6-pixel high glyph data.
type Face56vw struct {
	// Ascent is the glyph ascent, in pixels.
	Ascent int
	// Descent is the glyph descent, in pixels.
	Descent int
	// XHeight is the distance from the top of non-ascending lowercase
	// letters to the baseline, in pixels.
	XHeight int
	// Glyphs contains the glyphs.
	Glyphs []Glyph56vw
	// Ranges map runes to sub-images of Mask. The rune ranges must not
	// overlap, and must be in increasing rune order.
	Ranges []Range
	// Kernings maps rune pairs to kerning adjustments.
	Kernings map[string]int
}

// Close implements [io.Closer].
func (f *Face56vw) Close() error {
	return nil
}

// Glyph implements [font.Face].
func (f *Face56vw) Glyph(dot fixed.Point26_6, r rune) (
	dr image.Rectangle,
	mask image.Image,
	maskp image.Point,
	advance fixed.Int26_6,
	ok bool,
) {
	g, ok := f.GlyphFor(r)
	if ok {
		dr, mask, maskp, advance = g.draw(f, dot)
	}
	return
}

// GlyphBounds implements [font.Face].
func (f *Face56vw) GlyphBounds(r rune) (
	bounds fixed.Rectangle26_6,
	advance fixed.Int26_6,
	ok bool,
) {
	g, ok := f.GlyphFor(r)
	if ok {
		bounds, advance = g.bounds(f)
	}
	return
}

// GlyphAdvance implements [font.Face].
func (f *Face56vw) GlyphAdvance(r rune) (advance fixed.Int26_6, ok bool) {
	g, ok := f.GlyphFor(r)
	if ok {
		advance = g.advance()
	}
	return
}

// Kern implements [font.Face].
func (f *Face56vw) Kern(r0, r1 rune) fixed.Int26_6 {
	return fixed.I(f.Kernings[string([]rune{r0, r1})])
}

// UpdateKernings adds the values from seq to the face's kernings.
// If an adjustment already exists, its value will be overwritten.
func (f *Face56vw) UpdateKernings(seq iter.Seq2[string, int]) {
	if f.Kernings == nil {
		f.Kernings = make(map[string]int)
	}
	maps.Insert(f.Kernings, seq)
}

// Metrics implements [font.Face].
func (f *Face56vw) Metrics() font.Metrics {
	return font.Metrics{
		Height:     fixed.I(f.Ascent + f.Descent),
		Ascent:     fixed.I(f.Ascent),
		Descent:    fixed.I(f.Descent),
		XHeight:    fixed.I(f.XHeight),
		CapHeight:  fixed.I(f.Ascent),
		CaretSlope: image.Point{X: 0, Y: 1},
	}
}

// GlyphFor returns the glyph representing r, or the glyph representing
// the Unicode replacement character U+FFFD if no glyph specifically
// represents r.  It returns !ok if no glyph represents either r or the
// Unicode replacement character.
func (f *Face56vw) GlyphFor(r rune) (g Glyph56vw, ok bool) {
	index, ok := GlyphIndex(f.Ranges, r)
	if ok {
		g = f.Glyphs[index]
	}
	return
}
